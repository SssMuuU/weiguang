...
export default function Home() {
  ...
  const habitPartial = selectedHabits.filter((habit) => { const value = selectedRecord.habits[String(habit.id)] || 0; const target = habitStateFor(habit, selectedDate).target; return value > 0 && value < target; }).length;
  const habitContribution = selectedHabits.reduce((sum, habit) => sum + habitProgress(selectedRecord.habits[String(habit.id)] || 0, habitStateFor(habit, selectedDate).target).cappedPercent / 100, 0);
  ...
  function completionForDate(key: string) {
    ...
    const habitContribution = scheduled.reduce((sum, habit) => sum + habitProgress(record.habits[String(habit.id)] || 0, habitStateFor(habit, key).target).cappedPercent / 100, 0);
  ...
  const todayHabitStates = habits.filter((habit) => isScheduled(habit, todayKey)).map((habit) => habitProgress(recordFor(todayKey).habits[String(habit.id)] || 0, habitStateFor(habit, todayKey).target));
  ...
  function changeHabitProgress(id: number, amount: number, key = view === 'today' ? selectedDate : todayKey) {
    ...
    const nextProgress = habitProgress(nextValue, habitState.target);
    const crossedTarget = appliedAmount > 0 && currentValue < habitState.target && nextValue >= habitState.target;
    if (crossedTarget) { nativeSuccess(); wakeCompanion(`“${habit.title}”完成啦，尾巴都翘起来了。`); } else nativeImpact();
    if (key < todayKey) setToast(nextProgress.status === 'exceeded' ? `${habit.title} 已补记至 ${nextProgress.percent}%` : nextValue >= habitState.target ? `${habit.title} 已补记完成` : `${habit.title} 的历史进度已更新`);
    else setToast(nextProgress.status === 'exceeded' ? `${habit.title} 超额完成 ${nextProgress.percent}%` : nextValue >= habitState.target ? `${habit.title} 已完成` : appliedAmount > 0 ? `已记录 ${Math.abs(appliedAmount)} ${habitState.unit}` : `已减少 ${Math.abs(appliedAmount)} ${habitState.unit}`);
  }
