...
function NavIcon({ view }: { view: View }) {
  const paths: Record<View, string> = {
    today: 'M8 2v4m8-4v4M3 10h18M5 4h14a2 2 0 0 1 2 2v14H3V6a2 2 0 0 1 2-2M8 15l3 3 5-5',
    plans: 'M5 21V3m0 1h14l-3 5 3 5H5',
    habits: 'M20 7a9 9 0 0 0-15-2L2 8m0-5v5h5m-3 9a9 9 0 0 0 15 2l3-3m0 5v-5h-5M9 12l2 2 4-4',
    review: 'M4 3v17h17M8 16v-4m5 4V8m5 8V5',
    memos: 'M14 3H5a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-9M9 15l1-4L19 2l3 3-9 9-4 1',
  };
  return <svg width="21" height="21" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d={paths[view]} /></svg>;
}
...
export default function Home() {
  ...
  return (
    <main className="app-shell">
      ...
      <aside className="sidebar glass-panel"><button className="brand" onClick={() => navigate('today')} aria-label="微光首页"><span className="brand-mark">微</span><span>微光</span></button><nav className="nav-list" aria-label="主导航">{navItems.map((item) => <button key={item.id} className={`nav-item ${view === item.id ? 'active' : ''}`} onClick={() => navigate(item.id)} aria-current={view === item.id ? 'page' : undefined}><span><NavIcon view={item.id} /></span>{item.label}</button>)}</nav><div className="sidebar-foot"><div className="streak-orb">{Math.max(0, ...habits.map(habitStreak))}</div><div><strong>最长连续记录</strong><small>今天也向前一点</small></div></div></aside>
      ...
      <nav className="mobile-nav glass-panel" aria-label="移动端导航">{navItems.map((item) => <button key={item.id} className={view === item.id ? 'active' : ''} onClick={() => navigate(item.id)} aria-current={view === item.id ? 'page' : undefined}><span><NavIcon view={item.id} /></span>{item.label}</button>)}</nav>
